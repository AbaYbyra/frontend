...
export function validate_data(event){
  ...
  switch (event.target.id){
    ...
    case 'senha':
      if (event.target.value.length >= 6){
        ...
        if (event.target.value !== confirmSenha){
          message2 = 'As senhas não são iguais.'
        }
      } else{
 ...
 // console.log('message: ', message)
  return {campo: event.target.id, valid: valid, message: message, message2: message2}
} 
...
function meu_callback(conteudo) {
  if (!("erro" in conteudo)) {
      ...
      document.getElementById('nbhd').value=(conteudo.bairro);
